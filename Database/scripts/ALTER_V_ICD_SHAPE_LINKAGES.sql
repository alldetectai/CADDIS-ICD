CREATE OR REPLACE VIEW CADDIS_DEV.V_ICD_SHAPE_LINKAGES
( DIAGRAM_ID, DIAGRAM_NAME, LL_DIAGRAM_STATUS_ID, CREATED_BY, SHAPE_FROM_BIN_INDEX, 
 SHAPE_FROM_LABEL, SHAPE_FROM_ID, SHAPE_FROM_LABEL_SYMBOL, SHAPE_TO_LABEL, SHAPE_TO_LABEL_SYMBOL, 
 SHAPE_TO_ID, SHAPE_LINKAGE_ID, CITATION_ID, CAUSE_EFFECT_ID, DATASET_ID, IS_EFFECTRELATIONSHIP)
AS 
SELECT D.DIAGRAM_ID as DIAGRAM_ID,
          ...
          s.shape_label AS shape_from_label, sl.shape_from_id,
          s.ll_shape_label_symbol_id AS SHAPE_FROM_LABEL_SYMBOL,
          (SELECT s1.shape_label
             FROM icd_shape s1
            WHERE s1.shape_id = sl.shape_to_id) AS shape_to_label,
          (SELECT s1.ll_shape_label_symbol_id
             FROM icd_shape s1
            WHERE s1.shape_id = sl.shape_to_id) AS shape_to_label_symbol,
    ...
    --  AND (lcj.IS_EFFECTRELATIONSHIP IS NULL OR lcj.IS_EFFECTRELATIONSHIP = 'N' ) ;