...
--select * from P_CITATION where author like 'Admiraal W%'AUTHOR IS NOT NULL AND YEAR IS NOT NULL and

    FOR R IN (SELECT * FROM TMP_DATA_ENTRY  )
    LOOP
        IF R.CITATION_ID IS NOT NULL THEN
            V_CITATION_ID := R.CITATION_ID ;   
      
        END IF;
        
        IF R.SOURCE_DATA IS NOT NULL AND R.STUDY_TYPE IS NOT NULL THEN
           BEGIN
             
           /*
                SELECT CITATION_ID INTO V_CITATION_ID 
                  FROM P_CITATION
                 WHERE  UPPER(AUTHOR) =  UPPER(R.AUTHOR) 
                      AND YEAR = R.YEAR;
               */
              ...
              IF R.HABITAT IS NOT NULL THEN
               ...
               EXCEPTION
                ...
                INSERT INTO K_DATASET (DATASET_ID, CITATION_ID,  LL_SOURCE_DATA_ID, LL_STUDY_TYPE_ID, LL_HABITAT_ID, STUDYDETAILS,  LOCATION_COMMENT, CREATE_USER) 
                            VALUES(V_DATASET_ID, V_CITATION_ID, V_LL_SOURCE_DATA_ID,V_LL_STUDY_TYPE_ID, V_LL_HABITAT_ID, R.DESIGN_COMMENT, R.CONTEXT_COMMENT, 'CADDIS_DEV');
        ...
        IF R.CAUSE_TRAJECTORY IS NOT NULL THEN
          BEGIN
        ...
        IF R.EFFECT_TRAJECTORY IS NOT NULL THEN
         BEGIN
    ...
    --V_STATE_ENTITY_TYPE_ID
        ...
        DBMS_OUTPUT.PUT_LINE('AUTHOR IS ' || R.AUTHOR || ' YEAR ' || R.YEAR || ' Cause '  || R.CAUSE_TERM);
